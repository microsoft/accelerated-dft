...
import pyscf
def recreate_scf_obj(qcschema_dict,save_chk=False,chkfile=""):

    # Create Pyscf Molecule
    scf_dict, mol = load_qcschema_mol_scf(qcschema_dict,save_chk,chkfile,False)
    mol.build()
...
def prepare_ir(ks,hessian,freq):
    import pyscf
    from pyscf import gto, dft
    from pyscf.prop import infrared
    # make IR object but don't run
    ks_ir = infrared.rks.Infrared(ks) # check what type rks or uks
    # populate with hessian and other info we already have
    ks_ir.vib_dict = freq
    hessian.base = ks
    ks_ir.mf_hess = hessian
    # create orbital rotation deriv mo1_grad otherwise infrared will recompute hessian!
    h1ao_grad = ks_ir.mf_hess.make_h1(ks.mo_coeff, ks.mo_occ)
    moao1_grad, mo_e1_grad = ks_ir.mf_hess.solve_mo1(ks.mo_energy, ks.mo_coeff, ks.mo_occ, h1ao_grad)
    ks_ir._mo1_grad = pyscf.lib.einsum("up, uv, Axvi -> Axpi", ks.mo_coeff, ks.get_ovlp(), moao1_grad)
